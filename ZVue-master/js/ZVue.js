...
// 兼容es5中的属性定义方法，因为这是实现vue属性劫持的关键，如果不能兼容它，vue就无法实现
(function(){
    if(
        !Object.defineProperty || 
        !(function (){ try { Object.defineProperty({}, 'x', {}); return true; } catch (e) { return false; } }())
    ){
        // 获取原先的Object.defineProperty这个属性
        var orig = Object.defineProperty;
        Object.defineProperty = function (o, prop, desc) {
            // In IE8 try built-in implementation for defining properties on DOM prototypes.
            // 先在IE8中尝试使用内置实现来定义DOM原型的属性。
            if (orig){ 
                try{ return orig(o, prop, desc); } 
                catch(e){} 
            }
            // 在非对象上(NaN,undefined,null)调用Object.defineProperty.抛出错误
            if (o !== Object(o)){ 
                throw TypeError("Object.defineProperty called on non-object"); 
            }
            // 以下是对描述符get，set，value的兼容
            // 更早的IE版本实现描述符get和描述符set的方法要用非标准方法__defineGetter__和 __defineSetter__
            // 如果存在Object.prototype.__defineGetter__这方法并且使用了描述符get
            if (Object.prototype.__defineGetter__ && ('get' in desc)){
                // 通过借用函数__defineGetter__来实现描述符get的功能
                Object.prototype.__defineGetter__.call(o, prop, desc.get);
            }
            if (Object.prototype.__defineSetter__ && ('set' in desc)){
                Object.prototype.__defineSetter__.call(o, prop, desc.set);
            }
            // 如果存在描述符value，就改变对象的值
            if ('value' in desc){
                o[prop] = desc.value;
            }
            return o;
...
// 变异的数组构造函数
len_Vue.prototype._toNewArray = function(f_name, arr){

	// 实例具名回调函数(eval可以把传入的字符串当做js代码执行，它的返回值是执行完js代码的返回值)
	window.eval(
		"var callback = function "+f_name+" (args, k){"
		+"var temp = [];"
		+"for(var j=0;j<args.length;j++){temp.push(args[j])};"
		+"callback.vm.$data[k] = temp}"
	)
	callback.vm = this  // 给回调函数注入静态变量——ZVue的指针
	return new NewArray(arr, callback)  // 注入回调函数
}
...
// 属性劫持到变化，通知相应的watcher对象刷新view
len_Vue.prototype._defineProperty = function(obj, key, value, deep){

	Object.defineProperty(obj, key, {
		// configurable,configurable,get,set属于属性描述符
		// enumerable描述符：当该描述符的值为true时，该obj对象上的key属性的描述符的值才允许被改变
		configurable: true,
		// enumerable描述符：当该描述符的值为true时，该obj对象上的key属性才可以被枚举（遍历）
  		enumerable: true,
  		// get描述符：当该描述符的值为自定义的函数，当属性被读取或调用时调用该函数
  		get:function(){return value},
  		//  et描述符：当属性的值发生改变时，触发该函数，newVal是新的值，value是原本的值
  		set:function(newVal){
  			console.log(key+"进入了set钩子")
  			// 如果新值不等于原本的值
  			if(newVal !== value){
  				...
  				if(Number(key) >= 0){
  					// 数组元素变化，需要深度调用一层的映射表中的watcher对象刷新
  					this._render[deep][key]['_renders'].forEach(function(watcher){
  						return watcher.update()
  					})
  				}
  				else{
  					// 平级元素变化，直接调用映射表中的watcher对象刷新
					this._render[key]['_renders'].forEach(function(watcher){
						return watcher.update()
					})  								
  				}
  			}
...
function len_NewArray(arr,callback,callbackName){
	
	// super()调用 父对象/父类 的构造函数
	// Array()这个方法若只传一个参数，而且该函数是number类型，那它将会返回一个长度为该数值的空数组。
	// 如果参数没传回调函数，则返回一个普通数组
...
len_NewArray.prototype.push = function(){
	alert("监听推")
	Array.prototype.push.apply(this, arguments);
	this.callback(this, this.callback.name)  // 切面调用具名回调函数
}
len_NewArray.prototype.pop = function(){
	Array.pop() 
	this.callback(this, this.callback.name)
}
len_NewArray.prototype.splice = function(){
	Array.splice(...args)
	this.callback(this, this.callback.name)
}